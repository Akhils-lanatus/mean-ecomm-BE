...
const deleteImagesFromCloudinary = async (imagesArray: string[]) => {
    try {
        if (!imagesArray?.length) return null;
        let urlsWithoutExtension: string[] = [];
...
const deleteThumbnailFromCloudinary = async (localFilePath: string) => {
    try {
        if (!localFilePath) return null;
